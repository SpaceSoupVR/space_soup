...
impl Controllers {
    pub fn new(
        ...
        let left_grip_pose: xr::Action<xr::Posef> =
            action_set.create_action("left_grip_pose", "Left Grip Pose", &[])?;
        let right_grip_pose: xr::Action<xr::Posef> =
            action_set.create_action("right_grip_pose", "Right Grip Pose", &[])?;
        let left_aim_pose: xr::Action<xr::Posef> =
            action_set.create_action("left_aim_pose", "Left Aim Pose", &[])?;
        let right_aim_pose: xr::Action<xr::Posef> =
            action_set.create_action("right_aim_pose", "Right Aim Pose", &[])?;

        let left_trigger: xr::Action<f32> =
            action_set.create_action("left_trigger", "Left Trigger", &[])?;
        let right_trigger: xr::Action<f32> =
            action_set.create_action("right_trigger", "Right Trigger", &[])?;
        let left_squeeze: xr::Action<f32> =
            action_set.create_action("left_squeeze", "Left Squeeze", &[])?;
        let right_squeeze: xr::Action<f32> =
            action_set.create_action("right_squeeze", "Right Squeeze", &[])?;
        let left_thumbstick: xr::Action<xr::Vector2f> =
            action_set.create_action("left_thumbstick", "Left Thumbstick", &[])?;
        let right_thumbstick: xr::Action<xr::Vector2f> =
            action_set.create_action("right_thumbstick", "Right Thumbstick", &[])?;
        let left_thumbstick_click: xr::Action<bool> =
            action_set.create_action("left_thumbstick_click", "Left Thumbstick Click", &[])?;
        let right_thumbstick_click: xr::Action<bool> =
            action_set.create_action("right_thumbstick_click", "Right Thumbstick Click", &[])?;
        let left_thumbstick_touch: xr::Action<bool> =
    ...
    pub fn sync(
        ...
        self.state.l_trigger = self
            .left_trigger
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.r_trigger = self
            .right_trigger
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.l_squeeze = self
            .left_squeeze
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.r_squeeze = self
            .right_squeeze
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.l_stick = self
            .left_thumbstick
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.r_stick = self
            .right_thumbstick
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.l_stick_click = self
            .left_thumbstick_click
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.r_stick_click = self
            .right_thumbstick_click
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.l_stick_touch = self
            .left_thumbstick_touch
            .state(session, xr::Path::NULL)?
            .current_state;
        self.state.r_stick_touch = self
            .right_thumbstick_touch
            .state(session, xr::Path::NULL)?
            .current_state;
        ...
        self.state.l_grip_pose = l_grip
            .location_flags
            .contains(xr::SpaceLocationFlags::POSITION_VALID)
            .then_some(l_grip.pose);
        self.state.r_grip_pose = r_grip
            .location_flags
            .contains(xr::SpaceLocationFlags::POSITION_VALID)
            .then_some(r_grip.pose);
        self.state.l_aim_pose = l_aim
            .location_flags
            .contains(xr::SpaceLocationFlags::POSITION_VALID)
            .then_some(l_aim.pose);
        self.state.r_aim_pose = r_aim
            .location_flags
            .contains(xr::SpaceLocationFlags::POSITION_VALID)
            .then_some(r_aim.pose);
