...
use crate::xr::{XrContext, VkContext};
use crate::renderer::{
    camera::Camera,
...
impl XrRenderer {
    pub fn new(
        ...
        for &raw_image in &raw_images {
            let targets = std::array::from_fn(|eye| {
                let wgpu_tex = unsafe {
                    import_vk_image_as_wgpu(
                        &wgpu_device, raw_image,
                        wgpu_format, width, height, 2,
                    )
                };
                ...
                EyeTarget { _texture: wgpu_tex, view }
            });
    ...
    pub fn render_frame_with_meshes(
        ...
        let (_, eye_views) = session.locate_views(
            xr::ViewConfigurationType::PRIMARY_STEREO, time, stage,
        )?;

        let (solid_verts, solid_idx) = build_solid_mesh(cuboids);
        let (wire_verts,  wire_idx)  = build_wire_mesh(cuboids);

        let solid_vb = self.wgpu_device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("solid_vb"), contents: bytemuck::cast_slice(&solid_verts),
            usage: wgpu::BufferUsages::VERTEX,
        });
        let solid_ib = self.wgpu_device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("solid_ib"), contents: bytemuck::cast_slice(&solid_idx),
            usage: wgpu::BufferUsages::INDEX,
        });
        let wire_vb = self.wgpu_device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("wire_vb"), contents: bytemuck::cast_slice(&wire_verts),
            usage: wgpu::BufferUsages::VERTEX,
        });
        let wire_ib = self.wgpu_device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("wire_ib"), contents: bytemuck::cast_slice(&wire_idx),
            usage: wgpu::BufferUsages::INDEX,
        });
        ...
        for eye in 0..2usize {
            ...
            let mut encoder = self.wgpu_device.create_command_encoder(
                &wgpu::CommandEncoderDescriptor { label: Some("eye") },
            );
...
unsafe fn import_vk_image_as_wgpu(
    ...
    let hal_texture = hvk::Device::texture_from_raw(
        ...
        &wgpu::hal::TextureDescriptor {
            ...
            size:            wgpu::Extent3d { width, height, depth_or_array_layers: array_layers },
            mip_level_count: 1,
            sample_count:    1,
            dimension:       wgpu::TextureDimension::D2,
            format:          wgpu_format,
