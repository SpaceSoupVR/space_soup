...
//!
//! Scope: casts from world-space cuboids and non-skinned meshes. Skinned-mesh
//! casting and the VR (`xr_renderer`) shadow pass are follow-ups.
...
/// Side length of each (square) shadow depth texture.
...
impl ShadowSlot {
    fn new(device: &Device, light_layout: &BindGroupLayout, label: &str) -> Self {
...
impl ShadowMap {
    ...
    pub fn new(device: &Device) -> Self {
        ...
        let sun = ShadowSlot::new(device, &light_layout, "sun_shadow_depth");
        let spot = ShadowSlot::new(device, &light_layout, "spot_shadow_depth");
