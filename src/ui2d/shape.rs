...
impl Shape {
    ...
    pub fn positions(&self, offset: (f32, f32)) -> [[f32; 2]; 4] {
        ...
        let mut positions = [
            rotate(offset.0, offset.1),
            rotate(offset.0 + w, offset.1),
            rotate(offset.0, offset.1 + h),
            rotate(offset.0 + w, offset.1 + h),
        ];
        ...
        let shift = match self {
            Shape::Rectangle(_, _, _) | Shape::RoundedRectangle(_, _, _, _) => {
                let min = positions
                    .into_iter()
                    .reduce(|r, i| [r[0].min(i[0]), r[1].min(i[1])])
                    .unwrap();
