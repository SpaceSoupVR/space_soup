...
use crate::ui2d::shape::Shape;
use crate::ui2d::{Area, Color, RgbaImage};
use std::sync::Arc;
...
impl ShapeVertex {
    ...
    pub fn new(width: f32, height: f32, z: u16, area: Area, shape: Shape) -> [ShapeVertex; 4] {
        ...
        let [bx2, by2] =
            Self::transform_point(width, height, [bounds.0 + bounds.2, bounds.1 + bounds.3]);
...
impl RoundedRectangleVertex {
    pub fn new(
        ...
        shape: Shape,
        corner_radius: f32,
    ) -> [RoundedRectangleVertex; 4] {
        ShapeVertex::new(width, height, z, area, shape)
            .into_iter()
            .map(|shape| RoundedRectangleVertex {
                shape,
                corner_radius,
            })
            .collect::<Vec<_>>()
    ...
    pub fn new(shape: [V; 4], color: Color) -> [ColorVertex<V>; 4] {
        let c = |f: u8| {
            if f == 0 {
                ...
                (((f as f32 / u8::MAX as f32) + 0.055) / 1.055).powf(2.4)
            }
        };
        let color = [c(color.0), c(color.1), c(color.2), c(color.3)];
        shape
            .into_iter()
            .map(|shape| ColorVertex { shape, color })
            .collect::<Vec<_>>()
            .try_into()
            .unwrap()
    }
